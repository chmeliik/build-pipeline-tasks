...
For local tasks (tasks defined in this repo), we have two options; see below.
...
1. Not possible to update a task and a pipeline that references it in the same PR.
    ...
    - Similarly, it's not possible to introduce a new task and add it to the pipeline in the same PR.
